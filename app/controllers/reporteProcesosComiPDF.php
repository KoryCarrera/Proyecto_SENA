...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 1. Obtenemos el documento de la sesión del comisionado
    $documento_sesion = $_SESSION['user']['documento']; 
    $formato = 'PDF';
    $descripcion = 'Reporte Procesos Comisionado';

    $data = [
        ['value' => $documento_sesion, 'type' => PDO::PARAM_STR],
    ...
    if ($resProcesos && $datosInforme) {
        ...
        $listaCompleta = []; // Aquí guardaremos solo los del comisionado
        $totalActivos = 0;
        $totalInactivos = 0;

        foreach ($resProcesos as $proceso) {
            // Verificamos si el documento del proceso coincide con el de la sesión
            // Según tu SP, el documento del usuario se llama 'documento'
            if (isset($proceso['documento']) && $proceso['documento'] == $documento_sesion) {
                // Si coincide, lo agregamos a nuestra lista final
                $listaCompleta[] = $proceso;
                
                // Aprovechamos para contar los estados de una vez
                if ($proceso['estado'] == 1) {
                    $totalActivos++;
                } else {
                    $totalInactivos++;
        ...
        // 4. Calculamos porcentajes (evitando división por cero)
        ...
        $chartBarConfig = [
            ...
            'data' => [
                'labels' => ['Activos', 'Inactivos'],
                'datasets' => [[
                    'label' => 'Cantidad de Procesos', // <-- Cambiado
                    'data' => [$totalActivos, $totalInactivos],
                    'backgroundColor' => ['#39A900', '#dc3545']
                ]]
            ],
            'options' => [
                ...
                'title' => ['display' => true, 'text' => 'Estado de Mis Procesos'] // <-- Cambiado
            ]
        ];

        // Función para convertir la gráfica a Base64
...
?>
        ...
        <head>
            ...
            <style>
                ...
                .report-title { text-align: center; background-color: #2c3e50; color: white; padding: 10px; margin: 20px 0; font-weight: bold; }
                .charts-table { width: 100%; margin-bottom: 20px; border-collapse: collapse; }
                .chart-cell { width: 50%; text-align: center; padding: 10px; border: 1px solid #eee; }
                .chart-img { width: 250px; height: auto; }
                .stats-summary { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
                .stat-card { border: 1px solid #ddd; padding: 15px; text-align: center; width: 33.3%; }
                .stat-val { font-size: 18pt; font-weight: bold; color: #2c3e50; display: block; }
                .stat-lab { font-size: 8pt; color: #777; text-transform: uppercase; }
                .data-table { width: 100%; border-collapse: collapse; margin-top: 20px; }
                .data-table th { background-color: #2c3e50; color: white; padding: 8px; font-size: 9pt; text-align: left; }
                .data-table td { padding: 7px; border: 1px solid #ddd; font-size: 8.5pt; }
                .status-active { color: #39A900; font-weight: bold; }
        ...
        <body>
                ...
                <table style="width: 100%;">
                    <tr>
                        ...
                        <td style="text-align: right;">
                            ...
                            <div style="color: #666;">Reporte de Procesos del Comisionado</div> </td>
                    </tr>
                </table>
            </div>

            <div class='report-title'>ANÁLISIS DE PROCESOS ASIGNADOS</div> <table class="stats-summary">
                <tr>
                    <td class="stat-card">
                        <span class="stat-val"><?php echo $totalProcesos; ?></span>
                        <span class="stat-lab">Total Mis Procesos</span> </td>
                    <td class="stat-card">
            ...
            <table class="charts-table">
                <tr>
                    <td class="chart-cell">
                        ...
                        <?php else: ?>
                            <p style="color: red;">Error cargando gráfica</p>
                        <?php endif; ?>
                    </td>
                    <td class="chart-cell">
            ...
            <div style="background-color: #34495e; color: white; padding: 8px; margin-bottom: 10px; font-weight: bold;">
                2. LISTADO DETALLADO DE MIS PROCESOS </div>
            ...
            <table class='data-table'>
                ...
                <tbody>
                    <?php if(empty($listaCompleta)): ?>
                        <tr><td colspan="4" style="text-align:center;">No tienes procesos asignados actualmente.</td></tr> <?php else: ?>
...
<?php
        ...
        // <-- Nombre del archivo de descarga cambiado a Procesos
        $dompdf->stream("Reporte_Mis_Procesos_SENA.pdf", ["Attachment" => true]);
        exit;
    } else {
        echo "Hubo un error al obtener los procesos de la base de datos.";
    }
}
?>