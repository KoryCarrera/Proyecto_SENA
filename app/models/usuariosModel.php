...
class UsuariosModdel extends baseHelper
{
    ...
    public function validarExistenciaUser($documento, $correo, $nombre, $telefono)
    {
        ...
        try {
            ...
            $validateName = strtolower(str_replace(' ', '', $findUSer['nombre'] ?? '')) === $nombre;
            $validateCorreo = strtolower(str_replace(' ', '', $findUSer['email'] ?? '')) === $correo;
            $validatePhone = strtolower(str_replace(' ', '', $findUSer['numero'] ?? '')) === $telefono;

            if (!$validateName || !$validateCorreo || !$validatePhone) {
                throw new Exception('Los datos no coinciden con el usuario solicitado');
            };

            return true;
        } catch (Exception $e) {
            error_log('Error interno a la hora de validar usuario ' . $e->getMessage());
            return false;
        }
    }

    public function tokenRecuperacion($documento, $token) {
        ...
        try {
            ...
            throw new Exception('Ha ocurrido un error interno al guardar el token');
        }
    }

    public function actualizarContrasena($documento, $pass) {
        ...
        try {
            ...
            $dataPass = [
                [ 'value' => $documento, 'type' => PDO::PARAM_STR ],
                [ 'value' => $hashPass, 'type' => PDO::PARAM_STR ]
            ];

            parent::insertOrUpdateData('sp_actualizar_password_usuario(?, ?)', $dataPass);
        } catch (Exception $e) {
            error_log('Error al actualizar contraseña '. $e->getMessage());
            throw new Exception('Ha ocurrido un error interno al cambiar contrasena');
        }
    }
}
