...
const pendientes = document.getElementById('porAtender');
...
pendientes.textContent = `Cargando...`;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $.ajax({
        ...
        success: function (respuesta) {
            ...
            if (!total || !denuncia || !solicitudes || !tutelas || !atendidos || !pendientes) {
                Swal.fire({
                    icon: 'error',
            ...
            total.textContent = `${respuesta.status}`;
            total.textContent = `${respuesta.status}`;
            total.textContent = `${respuesta.status}`;
            total.textContent = `${respuesta.status}`;
            total.textContent = `${respuesta.status}`;
