...
async function cargarNotificaciones(url) {
  try {
    ...
    if (respuesta.status === 'ok') {
      ...
      $("#buscarComi").off("keyup").on("keyup", function () {
        dtInstance.search(this.value).draw();
      });

      // Filtro de cantidad
      $("#filtroCantidadComi").off("change").on("change", function () {
        const valor = parseInt($(this).val());
        dtInstance.page.len(valor).draw();
      });

      // Aseguramos que dataTables no le quite el fondo transparente al wrapper global de la tabla.
...
const actualizarPaginacionVisual = (table) => {
  ...
  const btnPrev = document.getElementById("btnPaginaAnteriorNotisComi");
  const btnNext = document.getElementById("btnPaginaSiguienteNotisComi");
  const contenedor = document.getElementById("pagBotonesNotisComi");
  ...
  if (totalPaginas > ventana && hasta - desde < ventana) {
    desde = Math.max(0, hasta - ventana);
  }
